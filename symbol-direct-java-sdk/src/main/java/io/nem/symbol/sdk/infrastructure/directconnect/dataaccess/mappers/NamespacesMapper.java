...
public class NamespacesMapper implements Function<JsonObject, NamespaceInfo> {
  ...
  public NamespaceInfo apply(final JsonObject jsonObject) {
    ...
    for (int i = 0; i < depth; i++) {
      levels.add(
          NamespaceId.createFromId(MapperUtils.toBigInteger(namespaceJsonObject, "level" + i)));
    }
    ...
    final BigInteger startHeight = MapperUtils.toBigInteger(namespaceJsonObject, "startHeight");
    ...
    return new NamespaceInfo(
        id,
        MapperUtils.getStateVersion(namespaceJsonObject),
        active,
        index,
        type,
        depth,
  ...
  public Alias getAlias(final JsonObject jsonObject) {
    ...
    switch (aliasType) {
      ...
      case MOSAIC:
        final MosaicId mosaicId = new MosaicId(MapperUtils.toBigInteger(aliasObject, "mosaicId"));
