...
public class MosaicGlobalRestrictionHelper extends BaseHelper<MosaicGlobalRestrictionHelper> {
    ...
    public MosaicRestriction<?> getMosaicRestriction(final MosaicId mosaicId) {
        return testContext.getRepositoryFactory().createRestrictionMosaicRepository().search(
                new MosaicRestrictionSearchCriteria().mosaicId(mosaicId)
                        .entryType(MosaicRestrictionEntryType.GLOBAL)).blockingFirst().getData().get(0);
    }
}
