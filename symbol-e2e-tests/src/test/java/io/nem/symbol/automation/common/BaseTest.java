...
public abstract class BaseTest {
  ...
  protected static void storeUserInfoInContext(
    ...
    if (!accountInfo.isPresent()) {
      ...
      final SupplementalAccountKeys supplementalAccountKeys =
          new SupplementalAccountKeys(null, null, null, new ArrayList<>());
      accountInfo =
          Optional.of(
              new AccountInfo(
                  "",
  ...
  protected BigInteger getActualMosaicQuantity(
    ...
    return testContext.getNetworkCurrency().getNamespaceId().get().getIdAsLong()
        ...
        ? Currency.CAT_CURRENCY.createRelative(amount).getAmount()
        : amount;
  }
  ...
  List<Mosaic> getValidMosaics(final List<MosaicNoCheck> mosaicNoChecks) {
    try {
      return mosaicNoChecks.stream()
          .map(m -> new Mosaic(m.getId(), m.getAmount()))
   ...
   */
  protected void transferAssets(
      final String sender, final String recipient, final List<Mosaic> mosaics) {
   ...
   */
  protected Optional<ResolvedMosaic> getMosaic(
      final AccountInfo accountInfo, final MosaicId mosaicId) {
