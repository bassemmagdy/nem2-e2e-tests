...
public class MosaicMetadata extends MetadataBase<UnresolvedMosaicId> {
  ...
  @And(
      "^(\\w+) request to update the \"(.+?)\" on (\\w+) asset \"(\\w+)\" with change of (-?\\d+) characters?$")
  ...
  @When(
      "^(\\w+) updates document \"(.+?)\" on asset \"(\\w+)\" with change of (-?\\d+) characters?$")
  ...
  @When("^(\\w+) adds document \"(\\w+)\" to asset \"(\\w+)\" using (\\w+) alias \"(\\w+)\"$")
  ...
  @Given(
      "^(\\w+) tries to add a document to asset \"(\\w+)\" without embedded in aggregate transaction$")
  ...
  public void createDocumentWithInvalidDelta(
      final String userName, final String documentName, final String assetName) {
  ...
  @Override
  protected MetadataTransaction createMetaTransaction(
      UnresolvedAddress targetAddress,
      BigInteger scopedMetadataKey,
      UnresolvedMosaicId targetId,
      short valueSizeDelta,
      String value) {
    return new MosaicMetadataHelper(getTestContext())
        .createMosaicMetadataTransaction(
            targetAddress, scopedMetadataKey, targetId, valueSizeDelta, value);
  }
  ...
  @Override
  protected MetadataTransaction getModifyTransaction(
